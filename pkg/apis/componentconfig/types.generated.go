...
func (x *KubeControllerManagerConfiguration) CodecEncodeSelf(e *codec1978.Encoder) {
	...
	if x == nil {
		...
		if false {
			...
			const yyr2 bool = false
			yyq2[50] = x.Kind != ""
			yyq2[51] = x.APIVersion != ""
			var yynn2 int
			if yyr2 || yy2arr2 {
				r.EncodeArrayStart(52)
			} else {
				yynn2 = 50
				for _, b := range yyq2 {
					if b {
...
func (x *KubeControllerManagerConfiguration) codecDecodeSelfFromArray(l int, d *codec1978.Decoder) {
	...
	if r.TryDecodeAsNil() {
		...
		yyv90 := &x.ResourceQuotaSyncPeriod
		yym91 := z.DecBinary()
		_ = yym91
		if false {
	...
	if r.TryDecodeAsNil() {
		...
		yyv94 := &x.PVClaimBinderSyncPeriod
		yym95 := z.DecBinary()
		_ = yym95
		if false {
	...
	if r.TryDecodeAsNil() {
		...
		yyv99 := &x.HorizontalPodAutoscalerSyncPeriod
		yym100 := z.DecBinary()
		_ = yym100
		if false {
	...
	if r.TryDecodeAsNil() {
		...
		yyv101 := &x.DeploymentControllerSyncPeriod
		yym102 := z.DecBinary()
		_ = yym102
		if false {
	...
	if r.TryDecodeAsNil() {
		...
		yyv107 := &x.NodeMonitorGracePeriod
		yym108 := z.DecBinary()
		_ = yym108
		if false {
	...
	if r.TryDecodeAsNil() {
		...
		yyv110 := &x.NodeStartupGracePeriod
		yym111 := z.DecBinary()
		_ = yym111
		if false {
	...
	if r.TryDecodeAsNil() {
		...
		yyv130 := &x.ControllerStartInterval
		yym131 := z.DecBinary()
		_ = yym131
		if false {
