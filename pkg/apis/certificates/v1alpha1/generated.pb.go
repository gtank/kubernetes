...
func init() {
	...
	proto.RegisterType((*Subject)(nil), "k8s.io.kubernetes.pkg.apis.certificates.v1alpha1.Subject")
}
...
func (m *CertificateSigningRequestSpec) MarshalTo(data []byte) (int, error) {
	...
	data[i] = 0x22
	i++
	i = encodeVarintGenerated(data, i, uint64(m.Subject.Size()))
...
func (m *Subject) MarshalTo(data []byte) (int, error) {
	...
	if len(m.ExtraNames) > 0 {
		for _, s := range m.ExtraNames {
			data[i] = 0x5a
			i++
			l = len(s)
			for l >= 1<<7 {
...
func (m *CertificateSigningRequestSpec) Unmarshal(data []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			m.ExtraInfo = append(m.ExtraInfo, string(data[iNdEx:postIndex]))
			iNdEx = postIndex
		case 3:
			if wireType != 2 {
...
func (m *Subject) Unmarshal(data []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			...
			m.StreetAddress = append(m.StreetAddress, string(data[iNdEx:postIndex]))
			iNdEx = postIndex
		case 7:
			if wireType != 2 {
			...
			m.PostalCode = append(m.PostalCode, string(data[iNdEx:postIndex]))
			iNdEx = postIndex
		case 8:
			if wireType != 2 {
		...
		case 11:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ExtraNames", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.ExtraNames = append(m.ExtraNames, string(data[iNdEx:postIndex]))
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipGenerated(data[iNdEx:])
