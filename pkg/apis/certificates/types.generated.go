...
import (
	...
	pkg4_unversioned "k8s.io/kubernetes/pkg/apis/certificates/unversioned"
...
func (x *CertificateSigningRequestSpec) CodecEncodeSelf(e *codec1978.Encoder) {
	...
	if x == nil {
		...
		if false {
			...
			yyq2[2] = x.Fingerprint != ""
			yyq2[3] = true
			yyq2[4] = len(x.Hostnames) != 0
			yyq2[5] = len(x.IPAddresses) != 0
			yyq2[6] = x.Username != ""
			yyq2[7] = x.UID != ""
			yyq2[8] = len(x.Groups) != 0
			var yynn2 int
			if yyr2 || yy2arr2 {
			...
			if yyr2 || yy2arr2 {
				...
				if false {
					...
					r.EncodeString(codecSelferC_UTF81234, string(x.Request))
				}
			} else {
				z.EncSendContainerState(codecSelfer_containerMapKey1234)
	...
	for yyj3 := 0; ; yyj3++ {
		...
		case "request":
			if r.TryDecodeAsNil() {
				...
				x.Request = string(r.DecodeString())
			}
		case "extrainfo":
			if r.TryDecodeAsNil() {
				x.ExtraInfo = nil
		...
		case "subject":
			if r.TryDecodeAsNil() {
				x.Subject = pkg4_unversioned.Subject{}
			} else {
				yyv8 := &x.Subject
				yym9 := z.DecBinary()
				_ = yym9
