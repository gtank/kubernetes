	...
	cc.csrStore.Store, cc.csrController = framework.NewInformer(
		&cache.ListWatch{
			ListFunc: func(options api.ListOptions) (runtime.Object, error) {
				return cc.kubeClient.Certificates().CertificateSigningRequests(api.NamespaceAll).List(options)
			},
			WatchFunc: func(options api.ListOptions) (watch.Interface, error) {
				return cc.kubeClient.Certificates().CertificateSigningRequests(api.NamespaceAll).Watch(options)
			},
		},
...
func (cc *CertificateController) updateCertificateRequestStatus(csr *certificates.CertificateSigningRequest) error {
	_, updateErr := cc.kubeClient.Certificates().CertificateSigningRequests(csr.Namespace).UpdateStatus(csr)
	if updateErr == nil {
		// success!
		return nil
	}
	// Update the set with the latest resource version for the next poll
	if _, getErr := cc.kubeClient.Certificates().CertificateSigningRequests(csr.Namespace).Get(csr.Name); getErr != nil {
...
func (cc *CertificateController) maybeSignCertificate(key string) error {
	...
	if csr.Status.Certificate == nil && IsCertificateRequestApproved(csr) {
		pemBytes, err := base64.StdEncoding.DecodeString(csr.Spec.Request)
		if err != nil {
			return err
		}
